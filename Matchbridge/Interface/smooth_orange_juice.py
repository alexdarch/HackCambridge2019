import numpy as np
from Interface.faceAPI import FaceAPI
import pandas as pd
import os
...
class SmoothOrangeJuice():
    def __init__(self): 
        ...
        # main image dataset
        self.dataset_dir = './static/images/main'
        self.dataset = self.get_dataset(init=False)
        # initial image dataset
        self.init_dir = './static/images/initial'
    ...
    def get_dataset(self, init=False):
        ...
        if init is False:
            print('getting dataset for main')
            files = os.listdir(self.dataset_dir)
            root = self.dataset_dir

        if init is True:
            print('getting dataset for initialisation')
        ...
        for i,f in enumerate(files): 
            ...
            print(self.similarity_average.iloc[i])
        return container
    ...
    def initialisation_run(self, response):
        # array of images to use for initial average calculation
        if len(self.image_set) is not self.count:
            image = self.image_set[self.count]
            # receive a +1 for YES, -1 for NO from user response
            self.update_similarity_average(image, response, init=True)

            return self.image_set[self.count+1]
        
        print("Initialisation finished")
        # self.Running is set true/false
        return None

    def main_run(self, response):
        if len(self.similarity_average) > 0:
